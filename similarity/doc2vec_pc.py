...
row_num = 0
workbook = xlsxwriter.Workbook('similarity_report_1.xlsx',{'nan_inf_to_errors': True})
...
for app_id in list(apps.keys())[:2000]:
    reviews_vecs = []
    releasenotes_vecs = []
...
# apps = pickle.load(open('apps_and_vecs.p','rb'))
...
for app_id in list(apps.keys())[:2000]:
    reviews_len = len(apps[app_id]['reviews'])
    if reviews_len:
        release_notes_mat = np.array(apps[app_id]['releasenotes_vecs']).reshape(
            (len(apps[app_id]['releasenotes_vecs']), size))
        reviews_mat = np.array(apps[app_id]['reviews_vecs']).reshape((len(apps[app_id]['reviews_vecs']), size))
        similarity = cosine_similarity(reviews_mat, release_notes_mat)
        releasenotes = apps[app_id]['release_notes']
        reviews = apps[app_id]['reviews']
